...
metriques_ipr_bv_dept <- ipr_numero_stat %>% 
  filter(dept %in% c("22", "28", "29", "35", "37", "41", "44", "45", "49", "50", "53", "56", "61", "72", "79", "85", "86")) %>%
  filter(obj_id %in% c("2", "3", "4", "5", "6", "15", "16")) #filtrer uniquement RCS, RCO, RCA, RHP, RRP et RCBL

# 2 - DCE ; 
### Regroupement ----

metriques_groupe_bv_dept <- metriques_ipr_bv_dept %>% 
  group_by(across(-lop_id)) %>% 
  summarise(across(!matches("lop_id"), first))

metriques_groupe_bv_dept <- metriques_groupe_bv_dept %>% 
  group_by(across(-pre_id)) %>% 
  summarise(across(!matches("pre_id"), first))

### Sélection de la date la plus récente par station ----

metrique_bv_date <- metriques_groupe_bv_dept %>% 
...
metrique_bv_date <- metrique_bv_date %>%
  mutate(sta_code_sandre = str_sub(sta_code_sandre, 4, 11)) 

station_ipr_metrique_bv_geom <- metrique_bv_date %>%
  left_join(station %>%
...
st_write(station_ipr_metrique_bv_geom,
         dsn = "data/outputs/station_ipr_metrique_bv_geom.gpkg")
