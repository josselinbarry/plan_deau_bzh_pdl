...
identifier_source_la_plus_proche_pour_un_bv <- function(id_bv,
                                                         sf_plandos,
                                                         sf_bv,
                                                         sf_sources)
{
  ...
  dist_df <- dist %>%
    as.data.frame() %>%
    mutate_all(as.numeric) %>%
    set_names(mes_sources %>% pull(cdoh_source)) %>%
    cbind(cdoh_plando = mes_pe %>% pull(cdoh_plando)) %>%
    pivot_longer(
      cols = -cdoh_plando,
      names_to = "cdoh_source",
      values_to = "distance"
    ) %>%
    group_by(cdoh_plando) %>%
    filter(distance == min(distance)) %>%
    mutate(distance_source=distance) %>%
    select(-distance)
...
identifier_source_la_plus_proche <- function(sf_plandos,
                                                        sf_bv,
                                                        sf_sources)
{
  mes_cdoh_bv <- unique(sf_bv$cdoh_bv)
  ...
  map_df(
    .x = mes_cdoh_bv,
    .f = identifier_source_la_plus_proche_pour_un_bv,
    sf_plandos = sf_plandos,
    sf_bv = sf_bv,
    sf_sources = sf_sources
  )
